...
function doInsert(){
	...
	if ($_POST['total'] == "" OR $_POST['exam'] == "") {
		...
		$grade = new Grades();
		$grade->CONTINOUS_ASSESSMENT  = $_POST['ca'];
		$grade->EXAMINATION   	= $_POST['exam'];
		$grade->TOTAL	= $_POST['total'];
		$grade->GRADE_LETTER 	= $_POST['gradeletter'];
		$grade->EQUIVALENT	= $_POST['equiv'];
		
		$grade->update($_GET['gradeid']);
			 	message("New grade added successfully!","success");
			 	redirect("index.php?view=coursestud&subjectid=".$_GET['subjectid']."&deptid=".$_GET['deptid']."&levelid=".$_GET['levelid']."&sem=".$_GET['sem']);
			 }
		}	 
	}
function doEdit(){
