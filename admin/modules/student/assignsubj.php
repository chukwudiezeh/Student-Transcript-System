	...
	<caption><h3 align="left">List of Course</h3></caption>		
		<div class="well">
				...
				<table id="example" class="table table-striped" cellspacing="0">
				  ...
				  <tbody>
					  ...
					  $mydb->setQuery("SELECT  * 
											...
											WHERE s.`LEVEL_ID`= l.`LEVEL_ID` AND s.`SUBJ_ID`=ss.`SUBJ_ID`AND ss.`IDNO`='{$studentId}' ");
							loadresult();
					  		$mydb->setQuery("SELECT  * 
					  						FROM  `subject` s,  `level` l WHERE s.`LEVEL_ID`= l.`LEVEL_ID` AND s.`SUBJ_CODE` 
											NOT IN (SELECT  `SUBJ_CODE` FROM  `subject` s,  `studentsubjects` ss WHERE  s.`SUBJ_ID`=ss.`SUBJ_ID` AND ss.`IDNO`='{$studentId}')  ");
					  	// $mydb->setQuery("SELECT  * 
				...
				<div class="btn-group">
				  <a href="index.php?view=subject&studentId=<?php echo (isset($studentId)) ? $studentId : 'ID' ;?>&cid=<?php echo (isset($levelid)) ? $levelid : 'cid' ;?>&sy=<?php echo (isset($sy)) ? $sy : 'sy' ;?>" class="btn btn-default"><span class="glyphicon glyphicon-back"></span>Back</a>
